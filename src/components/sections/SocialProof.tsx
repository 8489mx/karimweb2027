...
export function MobileSocialProofMarquee() {
  const { t } = useLanguage();
  ...
  return (
    <div className="md:hidden relative z-20 w-full max-w-[100vw] overflow-hidden py-2.5 sm:py-3 pb-[calc(0.65rem+env(safe-area-inset-bottom,0px))] border-t border-white/10 bg-black/40 backdrop-blur-xs flex mask-image-gradient pointer-events-none shrink-0">
      <div className="flex whitespace-nowrap animate-marquee w-fit">
        ...
        {[...stats, ...stats, ...stats, ...stats].map((stat, i) => (
          <div key={`stat-mobile-${i}`} className="flex items-center mx-4 sm:mx-6 shrink-0" dir={stat.value === 'ISSA' ? 'ltr' : 'rtl'}>
            <span className={`font-medium text-white tracking-wider text-[1.15rem] sm:text-[1.2rem] ${stat.value === 'ISSA' ? 'mr-2' : 'ml-2'}`}>
              {stat.value}
            </span>
            <span className="text-white/90 font-medium text-[0.8rem] sm:text-[0.85rem] uppercase tracking-wide whitespace-pre-line">
              {stat.label}
            </span>
          </div>
        ))}
