...
export function Footer() {
  ...
  return (
    <footer className="relative bg-transparent pt-2 md:pt-8 pb-20 md:pb-8 overflow-hidden">

      
      
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 lg:gap-4 mb-4 md:mb-6 items-center" dir={dir}>
          ...
          <div className="md:col-span-12 lg:col-span-6 flex flex-col items-center justify-center pt-4 lg:pt-0 h-full">
            <h3 className="hidden md:block text-[17px] font-bold text-slate-900 mb-3 font-sans text-center">
              {t.footer.contact}
            </h3>
            
            <div className="flex items-center gap-4">
              {(settings?.socialLinks?.facebook || (!settings?.socialLinks && "https://facebook.com/karimzakariia")) && (
              <a href={settings?.socialLinks?.facebook || "https://facebook.com/karimzakariia"} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-xl bg-slate-200/60 flex items-center justify-center text-slate-700 hover:-translate-y-1 hover:text-white hover:bg-[#1877F2] hover:shadow-lg hover:shadow-[#1877F2]/20 transition-all duration-200 active:scale-95" aria-label="Facebook">
              ...
              <a href={settings?.socialLinks?.snapchat || "https://snapchat.com/add/karimzakariia"} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-xl bg-slate-200/60 flex items-center justify-center text-slate-700 hover:-translate-y-1 hover:text-slate-900 hover:bg-[#FFFC00] hover:shadow-lg hover:shadow-[#FFFC00]/20 transition-all duration-200 active:scale-95" aria-label="Snapchat">
              ...
              <a href={settings?.socialLinks?.tiktok || "https://tiktok.com/@karimzakariia"} target="_blank" rel="noopener noreferrer" className="group w-10 h-10 rounded-xl bg-slate-200/60 flex items-center justify-center text-slate-700 hover:-translate-y-1 hover:text-white hover:bg-black hover:shadow-lg hover:shadow-black/20 transition-all duration-200 active:scale-95" aria-label="TikTok">
            ...
            <button onClick={() => setIsContactModalOpen(true)} dir="ltr" className="group flex items-center justify-center gap-3 w-full lg:w-auto mt-2 md:mt-4">
              <div className="w-10 h-10 rounded-xl bg-slate-200/60 flex items-center justify-center text-slate-700 group-hover:text-white group-hover:bg-brand-primary group-hover:shadow-lg group-hover:shadow-brand-primary/20 transition-all duration-200 active:scale-95 shrink-0">
        ...
        {/* Elegant Divider */}
        <div className="w-full h-[1px] bg-gradient-to-r from-transparent via-slate-200 to-transparent mb-4 md:mb-6" />

        {/* Legal & Copyright */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center" dir={dir}>
          <div className="lg:col-span-6 flex justify-center">
            <p className="text-slate-700 text-[12px] font-en tracking-wide text-center" dir="ltr">
              &copy; {new Date().getFullYear()} {t.footer.rights}
            </p>
          </div>

          <div className="lg:col-span-6 flex items-center justify-center order-first lg:order-last">
            {/* Legal policy links in one clean row */}
            <div className="flex items-center justify-center gap-3 sm:gap-4 md:gap-5 flex-nowrap">
              <Link to="/privacy-policy" className="text-slate-700 text-[12px] sm:text-[13px] font-bold md:font-medium hover:text-brand-primary transition-colors whitespace-nowrap">
                {t.footer.privacyPolicy}
              </Link>
              <span className="w-1 h-1 rounded-full bg-slate-300 shrink-0"></span>
              <Link to="/terms-and-conditions" className="text-slate-700 text-[12px] sm:text-[13px] font-bold md:font-medium hover:text-brand-primary transition-colors whitespace-nowrap">
                {t.footer.terms}
              </Link>
              <span className="w-1 h-1 rounded-full bg-slate-300 shrink-0"></span>
              <Link to="/refund-policy" className="text-slate-700 text-[12px] sm:text-[13px] font-bold md:font-medium hover:text-brand-primary transition-colors whitespace-nowrap">
